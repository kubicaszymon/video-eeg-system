...
configures**: for a peer implemented in file ``peer_a.py`` you define
...
Parameters defined in these sections can be overwritten in scenario file and/or
in additional configuration file loaded at the scenario start.
...
signal. Each peer is identified by ``peer_id``, which must be unique in the
...
define symbolic names for peers in ``[config_sources]`` section, from which
...
Here we just define internal (for this peer) names of configuration sources, hence empty spaces after equals sign.
In run-time real ``peer_id``s are assigned to these config sources using information stored in **scenario file**.

In a similar fashion we define ``[launch_dependencies]``. Launch dependencies
...
Now the ``[external_params]`` section. Let's say a peer needs a parameter
...
parameters between ``[external_params]`` and ``[local_params]`` in config files
...
When starting a peer with config support you need to provide a peer_id for it
and peer_ids of its launch/config dependecies. Custom config files -- option -f.
You can also override some parameters: using option -p / --local-params
param_name value or -e / --external-params param_name value_definitions.

Assume we want to run two peers: ``peer_a.py`` with default configuration
...
``peer_a`` takes parameter 'text' from a peer 'peerb', and ``peer_b`` takes
...
We do not need to provide launch dependency ID for peer_a because this time
it's the same peer as in config dependencies - assignment will be automatic.
...
:class:`~obci.core.configured_peer.ConfiguredPeer` class: ::

  from braintech.obci.core.broker.configured_peer import ConfiguredPeer
  class MyPeer(ConfiguredPeer)

Then you can use variables defined in config to initialize your peer in
:meth:`Peer._connections_established<obci.core.peer.Peer._connections_established>`
coroutine: ::

    class MyPeer(ConfiguredPeer):

        MANUAL_READY = False # Manually call self.ready() as part of initialization process
